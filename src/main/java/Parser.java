import componentsOfChecking.BracketChecker;
import componentsOfChecking.Checker;
public class Parser {
    ...
    public boolean transformFormula() {
        if (!Checker.startCheck(expression)) {
            return false;
        }
    ...
    private void transformInversion() {
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            if (character.compareTo(NOT) == 0) {
                ...
                if (BracketChecker.isBracketsNumberCorrect(subInversion)) {
